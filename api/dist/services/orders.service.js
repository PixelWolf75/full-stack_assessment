...
// -------------------------------------------------------
async function getOrder(id, query) {
    if (!id || isNaN(id))
        throw new Error("Valid order ID is required.");
    const { rows: orderRows } = await pool_1.default.query(`SELECT * FROM orders WHERE id = $1`, [id]);
    if (!orderRows.length)
        throw new Error(`Order with ID ${id} not found.`);
    const { rows: itemRows } = await pool_1.default.query(`SELECT
       oi.id,
       oi.order_id,
       oi.product_id,
       oi.qty,
    ...
    const total_cents = itemRows.reduce((sum, item) => sum + item.price_at_purchase * item.qty, 0);
...
async function createOrder(data) {
    ...
    try {
        ...
        for (const item of items) {
            const { rows } = await client.query(`SELECT id, name, stock_qty FROM products WHERE id = $1`, [item.product_id]);
            if (!rows.length) {
                throw new Error(`Product with ID ${item.product_id} not found.`);
            }
            if (rows[0].stock_qty < item.qty) {
                throw new Error(`Not enough stock for "${rows[0].name}". ` +
                    `Requested: ${item.qty}, available: ${rows[0].stock_qty}.`);
            }
        }
        // 2. Create the order record
        const { rows: orderRows } = await client.query(`INSERT INTO orders DEFAULT VALUES RETURNING *`);
        const order = orderRows[0];
        // 3. For each item: snapshot the current price, insert the
        //    order_item row, and reduce stock
        for (const item of items) {
            const { rows: productRows } = await client.query(`SELECT price_cents FROM products WHERE id = $1`, [item.product_id]);
            const price_at_purchase = productRows[0].price_cents;
            await client.query(`INSERT INTO order_items (order_id, product_id, qty, price_at_purchase)
         VALUES ($1, $2, $3, $4)`, [order.id, item.product_id, item.qty, price_at_purchase]);
